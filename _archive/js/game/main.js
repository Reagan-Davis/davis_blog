...
function initGame() {
  ...
  const achievementSystem = createAchievementSystem({ state, dom });
  ...
  function closeDungeonPanels(exceptPanel = null) {
    const panels = [dom.inventoryPanel, dom.achievementsPanel];
    const buttons = [dom.bagButton, dom.achievementsButton];
    ...
    buttons.forEach((button) => {
      ...
      const keepOpen = exceptPanel && controls === exceptPanel.id;
      button.setAttribute("aria-expanded", String(keepOpen));
      button.classList.toggle("is-active", Boolean(keepOpen));
    });
  }

  function setDungeonPanelOpen(panel, button, isOpen) {
    if (!panel || !button) return;

    if (isOpen) {
      closeDungeonPanels(panel);
      panel.hidden = false;
  ...
  dom.bagButton?.addEventListener("click", () => {
    ...
    setDungeonPanelOpen(dom.inventoryPanel, dom.bagButton, isHidden);
  ...
  dom.achievementsButton?.addEventListener("click", () => {
    ...
    setDungeonPanelOpen(dom.achievementsPanel, dom.achievementsButton, isHidden);
  ...
  dom.inventoryCloseBtn?.addEventListener("click", () => {
    setDungeonPanelOpen(dom.inventoryPanel, dom.bagButton, false);
    tutorialSystem.hideKeyHelp();
    tutorialSystem.hideViewportHelp();
  });

  dom.achievementsCloseBtn?.addEventListener("click", () => {
    setDungeonPanelOpen(dom.achievementsPanel, dom.achievementsButton, false);
  });
