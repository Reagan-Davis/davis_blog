...
function createRuntime() {
  ...
  runtime.emit = async function emit(eventName, payload = {}) {
    await runtime.triggerSystem.handleEvent(eventName, payload);
  };
...
function wireModalControls() {
  ...
  function openModal() {
    ...
    window.setTimeout(() => {
      const firstButton =
        modal.querySelector(".startup-dungeon-toolbar__button") ||
  ...
  modal.addEventListener("click", (event) => {
    ...
    if (target.dataset.closeModal !== "true") return;
    if (window.DungeonPrototypePanels?.isAnyOpen?.()) return;
    closeModal();
  });
...
function wireDungeonPanelToggles(runtime) {
  const buttons = document.querySelectorAll("[data-dp-panel-toggle]");
  const panels = document.querySelectorAll(".startup-dungeon-overlay");

  function getPanelDomId(panelName) {
    return PANEL_IDS[panelName] || null;
  }

  function setPanelOpenState(panelName, isOpen) {
    if (panelName === "inventory") {
      runtime.state.inventory.isOpen = isOpen;
    } else if (panelName === "editor") {
      runtime.state.editor.isOpen = isOpen;
    }
  }
  ...
  function closeAllPanels(exceptId = null) {
    ...
    for (const panelName of Object.keys(PANEL_IDS)) {
      setPanelOpenState(panelName, false);
    }
  }

  function openPanel(panelName) {
    const panelId = getPanelDomId(panelName);
    ...
    const button = document.querySelector(`[data-dp-panel-toggle="${panelId}"]`);
    button?.classList.add("is-active");
    setPanelOpenState(panelName, true);
    runtime.render();
    return true;
  }
  ...
  buttons.forEach((button) => {
    button.addEventListener("click", () => {
      const targetId = button.getAttribute("data-dp-panel-toggle");
      const panelName = Object.entries(PANEL_IDS).find(([, id]) => id === targetId)?.[0];
      if (!panelName) return;
  ...
  function isAnyPanelOpen() {
    return Array.from(panels).some((panel) => panel.classList.contains("is-open"));
  }

  window.DungeonPrototypeClosePanels = closeAllPanels;
  window.DungeonPrototypePanels = {
    open: openPanel,
    toggle: togglePanel,
...
async function start() {
  ...
  wireModalControls();
  wireDungeonPanelToggles(runtime);
  ...
  runtime.render();
  await runtime.emit("game_started", { sceneId: runtime.state.currentSceneId });
}
