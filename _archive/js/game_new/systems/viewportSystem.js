export function createViewportSystem(runtime) {
  ...
  async function interactWithObject(objectId, options = {}) {
    const scene = runtime.systems.scene.getCurrentScene();
    if (!scene) return;
    ...
    const previousSelected = runtime.state.inventory.selectedItemId;
    const droppedItemId = options.itemId || null;

    if (droppedItemId) {
      runtime.state.inventory.selectedItemId = droppedItemId;
    }
    ...
    await runtime.actionRunner.runActions(actionList, {
      sceneId: scene.id,
      objectId: objectDef.id,
      droppedItemId: droppedItemId || null
    });

    if (droppedItemId) {
    if (runtime.state.inventory.items.includes(droppedItemId)) {
        runtime.state.inventory.selectedItemId = previousSelected;
    } else {
